...
# MAGIC %md
# MAGIC ### Automated Result Analysis with DistilBERT+ViT fusion model - TEXT ONLY
# MAGIC
# MAGIC #### Intro: 
...
df = pd.read_parquet('/dbfs/mnt/s3_mount/qe_automation/data/merged_three_months.parquet')
...
df.to_parquet('/dbfs/mnt/s3_mount/qe_automation/data/merged_three_months.parquet', compression='brotli')
...
df = pd.read_parquet('/dbfs/mnt/s3_mount/qe_automation/data/data_optimized_proc.parquet')
df["CAT"] = df["IssueType"].map(lambda x: encode_label(x))
df

# COMMAND ----------

df.StepDescription[1]
...
for i, text in enumerate(df["StepDescription"]):
    for key in keys_to_remove:  
        text = re.sub(rf"{key}:.*?\n", "", text)  
    df2.at[i, "StepDescription"] = text
#rows_with_null =df2[df2.isnull().any(axis=1)] 
#rows_with_null
df2
...
class MultimodalFusionModel(nn.Module):
    ...
    def __init__(self, config, num_labels: int, intermediate_dims: int, dropout: float, pretrained_text_model: str, pretrained_image_model: str):
     ...
     #   self.image_encoder = AutoModel.from_pretrained(pretrained_image_model)#.to(self.device)
       ...
       # self.late_fusion = nn.Sequential(
        #    nn.Linear(in_features=self.text_encoder.config.hidden_size, out_features = intermediate_dims),
        #    nn.ReLU(),
        #    nn.Dropout(dropout), 
      #  )
        self.pred = nn.Linear(in_features = self.text_encoder.config.hidden_size, out_features = num_labels)
    ...
    def forward(
        ...
        pooler = encoded_text.last_hidden_state[:, 0, :]
     ...
     #  text_embeddings = self.gen_embeddings_sliding_window(input_ids, attention_mask)
      ...
      #      return_dict=True,
      #  )
       # fused_output = self.late_fusion(pooler)
        #    torch.cat(
         #       [
       #             pooler,
                   # text_embeddings,
...
# MAGIC %md
# MAGIC ### Train with Huggingface Trainer - NOT USED
...
# MAGIC %md
# MAGIC ### Finetune model with MLFlow - Current
    ...
    for batch_idx, data in enumerate(training_loader, 0):
      ...
      #      lens = data['lengths']
       ...
       #     pixel_values = data["pixel_values"].to(device)
            ...
            optimizer.zero_grad()
            outputs = model(ids, None, mask, None, targets)
            loss = outputs["loss"]
...
def clean_and_parse(text, keys_to_remove):
    for key in keys_to_remove:  
        text = re.sub(rf"{key}:.*?\n", "", text) 
    return text
...
def prep_data(config: Dict):
    ...
    # convert to a cudf dataframe to parallelize dataframe ops on gpu
...
def train_classifier(config):
    ...
    experiment_id = init_experiment("/Users/Chris.Jose@cvent.com/test_result_analysis/experiments/test_result_analysis_3")
...
# MAGIC %md Entrypoint for Training/Evaluation
...
#tmp_model, tokenizer, testing_set = mp.spawn(train_classifier, args=(world_size, config), nprocs=world_size)
train_classifier(config)

# COMMAND ----------

# MAGIC %md Convert to Torchscript

# COMMAND ----------

with open(CONF_PATH) as conf_f:
    config = yaml.safe_load(conf_f)
model = initialize(config, config["data"]["num_labels"])
model.eval()

traced_script_module = torch.jit.trace(model, (ids, None, mask, None, targets))
traced_script_module.save(config["train"]["model_repo"])
